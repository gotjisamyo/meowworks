...
export default function IndustryPage({ industry }: Props) {
  ...
  return (
    <Layout>
      <SEO title={industry.metaTitle} description={industry.metaDescription} canonical={canonical} faqSchema={faqSchema} />
        ...
        <div className="max-w-4xl mx-auto px-4 py-16 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              ...
              onClick={() => trackCTA({ location: `industry_hero_${industry.slug}`, label: PRIMARY_CTA_LABEL, destination: PRIMARY_CTA_HREF, variant: 'primary' })}
      ...
      <section className="py-16 max-w-4xl mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-black text-center mb-10">ปัญหาที่{industry.industry}มักเจอ และ workflow ที่ MeowChat ช่วยได้</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {industry.painPoints.map((p, i) => (
            <div key={i} className="bg-brand-card rounded-2xl p-5 border border-white/5">
              <div className="w-8 h-8 bg-red-500/10 border border-red-500/20 rounded-full flex items-center justify-center text-red-400 font-bold text-sm mb-3">!</div>
      ...
      <section className="py-16 bg-brand-card/30">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-black text-center mb-10">ตัวอย่างความสามารถที่ออกแบบมาสำหรับ{industry.industry}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      ...
      <section className="py-16 max-w-2xl mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-black text-center mb-10">ตัวอย่างการสนทนา</h2>
        <div className="bg-brand-card rounded-2xl border border-white/5 overflow-hidden">
          <div className="bg-[#151520] px-4 py-3 flex items-center gap-3 border-b border-white/5">
            ...
            <div>
              ...
              <div className="text-xs text-[#00B900]">● พร้อมช่วยตอบตาม workflow ที่ตั้งค่าไว้</div>
            </div>
          </div>
          <div className="p-4 space-y-3">
            {industry.demo.map((msg, i) => (
              <div key={i} className={`flex ${msg.from === 'customer' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] px-4 py-2 rounded-2xl text-sm leading-relaxed ${msg.from === 'customer' ? 'bg-brand-orange text-white' : 'bg-white/5 text-white/80'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
      ...
      {industry.faq.length > 0 && (
        <section className="py-16 max-w-3xl mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-black text-center mb-10">คำถามที่พบบ่อยสำหรับ{industry.industry}</h2>
          <div className="space-y-3">
            {industry.faq.map((item, i) => (
              <details key={i} className="bg-brand-card border border-white/5 rounded-xl group">
                ...
                <div className="px-5 pb-4 text-white/50 text-sm leading-relaxed border-t border-white/5">{item.a}</div>
              </details>
            ))}
          </div>
        </section>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 text-center">
        <div className="flex flex-wrap justify-center gap-2">
          {industry.keywords.slice(0, 8).map((kw) => (
            <span key={kw} className="text-xs bg-white/5 text-white/30 px-3 py-1 rounded-full">{kw}</span>
          ))}
        </div>
      </div>
